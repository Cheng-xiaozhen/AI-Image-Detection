...
class DINOv3Vith16VIB(nn.Module):
    ...
    def forward(self, pixel_values: torch.Tensor, labels=None) -> ImageClassifierOutput:
        ...
        # 2. 通过 VIB Head 获取预测值和 KL 损失
        head_outputs = self.head(feats)
        logits = head_outputs["logits"]
        aux_loss = head_outputs["aux_loss"]

        loss = None
        if labels is not None:
            # 3. 计算主分类任务损失
            bce_loss = self.loss_fct(logits.view(-1), labels.float().view(-1))
            
            # 4. 训练时合并 VIB 约束，评估时只保留分类损失
            loss = bce_loss + aux_loss if self.training else bce_loss
